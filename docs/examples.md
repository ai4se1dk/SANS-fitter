...
into this repository: they can be found inside the installed `sasdata` package, which
is already a dependency. That keeps the wheel small and the collection in
sync with sasdata.
...
The starting values are coarse: chosen to put the optimiser in the right area,
...
!!! note "`cylinder` is a verification dataset, not a fitting exercise"
    ...
    check the model pipeline against. For a cylinder
